...
# Endpoint config lives in .env at the bench root so the judge model can use a
# different endpoint than the agent model; real environment variables win.
...
def eval_result(input_path, output_path):
    ...
    """
    judge = ChatOpenAI(model=JUDGE_MODEL, temperature=0, api_key=JUDGE_API_KEY, base_url=JUDGE_BASE_URL)
    evaluator = load_evaluator("cot_qa", llm=judge)
    data_list = []
    visited = []
        ...
        for attempt in range(max_retries):
            try:
                return evaluator.evaluate_strings(
                    prediction=data["pred"], input=data["question"], reference=data["answer"]
                )
    ...
    # LLM judge calls commonly take 5-30s each; the original timeout=4 dropped
    # nearly every slow scoring call silently. Configurable via EVAL_TIMEOUT.
    result_timeout = int(os.getenv("EVAL_TIMEOUT", "300"))
    ...
    with tqdm(total=len(data_list)) as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_data):
                try:
                    outputs = future.result(timeout=result_timeout)
...
if __name__ == "__main__":
    ...
    if not JUDGE_API_KEY or not JUDGE_BASE_URL:
        raise SystemExit("Please set JUDGE_API_KEY and JUDGE_BASE_URL (in .env or the environment).")
