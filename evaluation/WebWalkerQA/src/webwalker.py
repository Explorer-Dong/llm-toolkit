...
_REACT_MARKER_BOLD = re.compile(r"\*\*\s*(Thought|Action|Action Input|Observation|Final Answer)\s*:\s*\*\*")
_REACT_MARKER_HEADING = re.compile(r"#{1,4}\s*(Thought|Action|Action Input|Observation|Final Answer)\s*:")


def normalize_react_markers(text: str) -> str:
    """Some models wrap ReAct markers in markdown bold (``**Action:**``) or
    headings (``### Action:``), which the strict '\\nAction:' detection below
    would miss entirely."""
    text = _REACT_MARKER_BOLD.sub(r"\1:", text)
    return _REACT_MARKER_HEADING.sub(r"\1:", text)
...
class WebWalker(FnCallAgent):
    ...
    def critic_information(self, query, memory):
        ...
        response = self.client.chat.completions.create(
            model=self.llm_cfg["model"], response_format={"type": "json_object"}, messages=messages
        )
        max_retries = 10
        for attempt in range(max_retries):
            try:
    ...
    def _run(self, messages: List[Message], lang: Literal["en", "zh"] = "en", **kwargs) -> Iterator[List[Message]]:
        ...
        while num_llm_calls_available > 0:
            ...
            if not output:  # empty LLM response (rare); re-ask instead of crashing on output[-1]
                continue
            if output:
            ...
            if not has_action:
                if "Final Answer: " in normalize_react_markers(output[-1].content):
                    break
                else:
                    continue
    ...
    def _detect_tool(self, text: str) -> Tuple[bool, str, str, str]:
        ...
        elif i >= 0 and j < 0 and len(self.function_map) == 1:
            # Degenerate form: "Action:" followed directly by a JSON body
            # (often fenced, e.g. ```json {"button": ...} ```), without the
            # "Action Input:" line at all. Single-tool setup: infer the name.
            match = re.search(r"\{.*\}", text[i + len(special_func_token) :], re.S)
            if match:
                func_name = next(iter(self.function_map))
